...
BROKER_NAME = 'broker' + subprocess.check_output(" awk 'END{print $1}' /etc/hosts ", encoding='utf-8',stderr=subprocess.STDOUT, shell=True).strip().replace(".", "")
print("Broker ID", BROKER_NAME)
...
class Broker:
    ...
    def match_and_notify_subscriber(self, message, addr):
        if message.id not in self.publish_history:
            self.publish.publish_event(message.event, message.message, BROKER_NAME)
    ...
    def process_message(self, msg, addr):
        if msg.action == msg_util.SUBSCRIBE:
            print(" MSG from ", addr[0], "action", msg.action, "ttl", msg.ttl)
            ...
            print("not identifies", msg.__dict__)
